...
const Product = (props) => {
  const { item } = props
  return (
    <div className="flex flex-col border-1 my-2 py-2 border-gray-400 bg-white rounded-lg h-76 w-64 p-2 m-2">
      <div className="flex justify-center">
        <img className="card__image h-32" src={item.image} alt={item.description} />
      </div>
      <div className=" pb-0 mb-0 pt-4 h-24 px-2 py-2 font-light text-l mb-2">
        {item.title}
